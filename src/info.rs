...
pub(crate) fn info(){
    ...
    let tst = Command::new("python3").arg("-c").arg("import os; os.system('echo -ne \"\\033]30;TAM RUSTy\\007\"')").stderr(std::process::Stdio::piped()).stdout(std::process::Stdio::piped()).output();
    clear_screen();
   // std::io::stdout().write_all(&tst.unwrap().stdout);
    println!("{}", crate::from_utf8(&tst.unwrap().stderr).unwrap());
