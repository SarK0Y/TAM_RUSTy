use crate::{clear_screen, getkey, repeat_char, run_cmd_out_sync, run_cmd_str, STRN};
use std::io::Write;
use std::os::fd::FromRawFd;
use std::process::Command;
#[macro_use]
pub fn Ver0_0_() -> String {
    let ver = option_env!("PROJECT_VERSION")
        .unwrap_or(env!("CARGO_PKG_VERSION"))
        .to_string();
    let ver = format!("Ver: {ver}");
    ver
}
pub(crate) fn SYS() {
...
pub(crate) fn info() {
    ...
    banners_line(Project, "◑");
    println!("");
    banners_line(&Ver0_0_(), "◑");
    println!("");
    banners_line(Telega, "◑");
    println!("");
    banners_line(Ru_blog, "◑");
    println!("");
    banners_line(En_blog, "◑");
    println!("");
    banners_line(ChangeLog, "◑");
    println!("");
    banners_line(Forum, "◑");
    println!("");
    banners_line(E_mail, "◑");
    ...
    banners_line(Github, "◑");
    println!("");
    banners_line(YouTube, "◑");
    println!("");
    banners_line(Twitter_X, "◑");
    println!("");
    banners_line(Supported_platforms, "◑");
    println!("");
    println!("{}", LICENSE);
    #[cfg(feature = "mae")]
    println!("{}", LICENSE_MAE);
...
pub(crate) fn banners_line(val: &str, filler: &str) {
    ...
    if w_half_size + 1 > val_half_size {
        w_half_size -= (val_half_size)
    }
...
pub(crate) fn KonsoleTitle(title: &String) {
    ...
    let writeTermTitle = Command::new("python3")
        .arg("-c")
        .arg(title)
