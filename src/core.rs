...
use crate::{
    ...
    run_cmd_out_sync, run_cmd_str, session_lists, set_ask_user, set_full_path,
    shift_cursor_of_prnt, split_once,
...
use self::ps21::{get_mainpath, get_prnt, get_tmp_dir};
core_use!();
...
pub(crate) fn complete_path(dir: &str, opts: &str, no_grep: bool) {
    ...
    //}
    if prnt.len() == 0 {
        crate::set_ask_user("prnt is empty", -47);
    }
    if num_of_ln_in_dir_lst < 2 {
        ...
        crate::set_prnt(&prnt, -47);
        let prnt = read_prnt();
        crate::set_ask_user(&prnt, -47);
