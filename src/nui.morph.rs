...
pub fn mk_morph_alg24_rev ( samples: &mut [f32], uv: &crate::enums::universum_vox_morph ){
    let mut ch:  Vec < Vec <f32> > = Vec::new (); ch.push ( vec! () ); ch.push ( vec! () );
    let mut count_steps = 0usize; 
    let mut sel_ch: usize = uv.select_channel.unwrap_or (0) as usize;
    let range = uv.step_factor as usize;
    let half_range = range >> 1;
    loop {
        let mut samples_to_morph: Vec <_> = read_chan_f32(samples, sel_ch, 2, count_steps, range)
            .into_iter()
            .rev()
            .collect();
...
pub fn mk_morph_alg26_wave_energy(samples: &mut [f32], uv: &crate::enums::universum_vox_morph ) {
...
pub fn mk_morph_alg31_shuffle(samples: &mut [f32], uv: &crate::enums::universum_vox_morph ) {
    use crate::faav::over_uv;
    use crate::faav::over_samples;
    use crate::faav::over_samples0;
    let over_uv_: *const crate::enums::universum_vox_morph = uv;
...
pub fn mk_morph_alg35_wave_energy_simple_smooth(samples: &mut [f32], uv: &crate::enums::universum_vox_morph ) {
    use crate::faav::over_uv;
    use crate::faav::over_samples;
    use crate::faav::over_samples0;
    let over_uv_: *const crate::enums::universum_vox_morph = uv;
...
pub fn mk_morph_alg36_wave_energy_norma(samples: &mut [f32], uv: &crate::enums::universum_vox_morph ) {
    use crate::faav::over_uv;
    use crate::faav::over_samples;
    use crate::faav::over_samples0;
    let over_uv_: *const crate::enums::universum_vox_morph = uv;
...
pub fn mk_morph_alg37_wave_energy_vox(samples: &mut [f32], uv: &crate::enums::universum_vox_morph ) {
    use crate::faav::over_uv;
    use crate::faav::over_samples;
    use crate::faav::over_samples0;
    let over_uv_: *const crate::enums::universum_vox_morph = uv;
...
pub fn mk_morph_alg39_wave_energy_mix_poly(samples: &mut [f32], uv: &crate::enums::universum_vox_morph ) {
    use crate::faav::over_uv;
    use crate::faav::over_samples;
    use crate::faav::over_samples0;
    let over_uv_: *const crate::enums::universum_vox_morph = uv;
    crate::faav::over_uv( Some (over_uv_ ) );
     ...
     over_samples(Some (over_ch0) );
    let mut thr1 = std::thread::spawn (move|| 
        {crate::cdsp::tune_wave_energy_mix_poly (
           unsafe { &mut *over_samples(None).unwrap() },
           unsafe { &*over_uv(None).unwrap() } ); });
     let mut ch1: Vec <_> = read_chan_f32(samples, 1, 2, 0, samples.len() );
     let mut over_ch1: *mut [f32] = &mut *ch1;
...
pub fn mk_morph_alg40_wave_energy_mix_log_norma(samples: &mut [f32], uv: &crate::enums::universum_vox_morph ) {
    use crate::faav::over_uv;
    use crate::faav::over_samples;
    use crate::faav::over_samples0;
    let over_uv_: *const crate::enums::universum_vox_morph = uv;
    crate::faav::over_uv( Some (over_uv_ ) );
     ...
     over_samples(Some (over_ch0) );
    let mut thr1 = std::thread::spawn (move|| 
        {crate::cdsp::tune_wave_energy_mix_log_norma (
           unsafe { &mut *over_samples(None).unwrap() },
           unsafe { &*over_uv(None).unwrap() } ); });
     let mut ch1: Vec <_> = read_chan_f32(samples, 1, 2, 0, samples.len() );
     let mut over_ch1: *mut [f32] = &mut *ch1;
...
pub fn mk_morph_alg41_wave_energy_mix_sin_norma(samples: &mut [f32], uv: &crate::enums::universum_vox_morph ) {
    use crate::faav::over_uv;
    use crate::faav::over_samples;
    use crate::faav::over_samples0;
    let over_uv_: *const crate::enums::universum_vox_morph = uv;
...
pub fn mk_morph_alg30_wave_energy_mix(samples: &mut [f32], uv: &crate::enums::universum_vox_morph ) {
    use crate::faav::over_uv;
    use crate::faav::over_samples;
    use crate::faav::over_samples0;
    let over_uv_: *const crate::enums::universum_vox_morph = uv;
    crate::faav::over_uv( Some (over_uv_ ) );
     ...
     over_samples(Some (over_ch0) );
    let mut thr1 = std::thread::spawn (move|| 
        {crate::cdsp::tune_wave_energy_mix1 (
           unsafe { &mut *over_samples(None).unwrap() },
           unsafe { &*over_uv(None).unwrap() } ); });
     let mut ch1: Vec <_> = read_chan_f32(samples, 1, 2, 0, samples.len() );
     let mut over_ch1: *mut [f32] = &mut *ch1;
...
pub fn mk_morph_alg27_wave_energy(samples: &mut [f32], uv: &crate::enums::universum_vox_morph ) {
    use crate::faav::over_uv;
    use crate::faav::over_samples;
    use crate::faav::over_samples0;
    let over_uv_: *const crate::enums::universum_vox_morph = uv;
...
pub fn mk_morph_alg23_wave_energy(samples: &mut [f32], uv: &crate::enums::universum_vox_morph ) {
    use crate::faav::over_uv;
    use crate::faav::over_samples;
    use crate::faav::over_samples0;
    let over_uv_: *const crate::enums::universum_vox_morph = uv;
...
pub fn mk_morph_alg21_replace_freqs(samples: &mut [f32], uv: &crate::enums::universum_vox_morph ) {
    ...
    crate::faav::over_uv( Some (over_uv_ ) );
     ...
     over_samples(Some (over_ch0) );
    let mut thr1 = std::thread::spawn (move|| 
        {mk_morph_alg21_replace_freqs_ (
           unsafe { &mut *over_samples(None).unwrap() }, unsafe { &*over_uv(None).unwrap() } ); });
     let mut ch1: Vec <_> = read_chan_f32(samples, 1, 2, 0, samples.len() );
     let mut over_ch1: *mut [f32] = &mut *ch1;
     over_samples0(Some (over_ch1) );
     let mut thr2 = std::thread::spawn (move|| {
        mk_morph_alg21_replace_freqs_ (
            unsafe { &mut *over_samples0(None).unwrap() }, unsafe { &*over_uv(None).unwrap() } ); });
    thr1.join();
    thr2.join();
    write_chan_f32(samples, 0, 2, 0, &ch0 );
    write_chan_f32(samples, 1, 2, 0, &ch1 );
...
pub fn mk_morph_alg21_replace_freqs_(samples: &mut [f32], uv: &crate::enums::universum_vox_morph ) {
    ...
    let scale = uv.scale.unwrap_or (0.8);
    let bar = uv.bar_sample;
    for j in 0..check_freq.len() / 2 {
        let indx = 2 * j;
        let old_range = &check_freq[indx];
        let new_range = &check_freq[indx + 1];
        for from in 0..samples.len() {
            crate::cdsp::replace_freqs_component_from_sample_(&mut samples [from], from, old_range, new_range, scale, bar);
