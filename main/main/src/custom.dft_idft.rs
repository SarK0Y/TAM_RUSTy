...
pub fn wave_energy_vox (samples: &mut [f32], uv: &crate::enums::universum_vox_morph) {
    ...
    for j in from..to {
        if samples [ j ].abs () > new_top.abs () {
            while samples [ j ].abs () > new_top.abs () {
                samples [ j ] *= scale;
            } new_top = samples [j];
        } 
...
pub fn wave_energy_hills (samples: &mut [f32], uv: &crate::enums::universum_vox_morph) {
    ...
    println! ("Ended wave_energy_hills");
}
pub fn wave_energy_log (samples: &mut [f32], uv: &crate::enums::universum_vox_morph) {
    if uv.input_u64.is_none() {errMsg0("'input_u64' in Universum Vox must be set"); return}
    if uv.input_f32.is_none() {errMsg0("'input_f32' in Universum Vox must be set"); return}
    let input_u64 = uv.input_u64.clone().unwrap();
    if input_u64.len() < 2 {errMsg0("'input_u64' in Universum Vox sets 'from' & 'to'"); return}
    ...
    for j in from..to {
       ...
       // let exp_to_scale = (ratio / scale_ratio).log( scale_ratio );  
        /* while cur_ratio < ratio {
            cur_ratio *= scale_ratio;
        } */ samples [ j ] = base_sample.sign() * (base_sample.abs ().log(samples [j].abs() ).powf(ratio) * scale_ratio ) % 0.9999;
        base_sample = samples [ j - 1];
    }
...
pub fn wave_energy_poly (samples: &mut [f32], uv: &crate::enums::universum_vox_morph) {
    ...
    for j in from..to {
        ...
        samples [ j ] = samples [j].sign () * crate::cmath::poly:: < f32 >(vex, samples [j] );
    }
    println! ("Ended wave_energy_poly");
}
pub fn wave_energy_log_norma (samples: &mut [f32], uv: &crate::enums::universum_vox_morph) {
    if uv.input_u64.is_none() {errMsg0("'input_u64' in Universum Vox must be set"); return}
    if uv.vex_f32.is_none() {errMsg0("'vex_f32' in Universum Vox must be set"); return}
    if uv.input_f32.is_none() {errMsg0("'input_f32' in Universum Vox must be set"); return}
    let input_u64 = uv.input_u64.clone().unwrap();
...
pub fn wave_energy_sin_norma (samples: &mut [f32], uv: &crate::enums::universum_vox_morph) {
    if uv.input_u64.is_none() {errMsg0("'input_u64' in Universum Vox must be set"); return}
   // if uv.vex_f32.is_none() {errMsg0("'vex_f32' in Universum Vox must be set"); return}
    if uv.input_f32.is_none() {errMsg0("'input_f32' in Universum Vox must be set"); return}
    let input_u64 = uv.input_u64.clone().unwrap();
    ...
    let to = if input_u64 [1] == 0 { samples.len() } else {input_u64 [2] as usize };
    let input_f32 = uv.input_f32.clone().unwrap();
    if input_f32.len() < 3 {errMsg0("'input_f32' in Universum Vox sets 'inner_scale' & 'amp', 'max_bottom'"); return}
    let inner_scale = input_f32 [0];
    let amp: f32 = input_f32 [ 1 ];
    let max_bottom: f32 = input_f32 [ 2 ];
   // let vex: &Vec < f32 > = &uv.vex_f32.clone().unwrap() [0];
    for j in from..to {
        //if samples [j].abs() <= max_bottom{continue;}
...
pub fn wave_energy_pow (samples: &mut [f32], uv: &crate::enums::universum_vox_morph) {
    if uv.input_u64.is_none() {errMsg0("'input_u64' in Universum Vox must be set"); return}
   // if uv.vex_f32.is_none() {errMsg0("'vex_f32' in Universum Vox must be set"); return}
    if uv.input_f32.is_none() {errMsg0("'input_f32' in Universum Vox must be set"); return}
    let input_u64 = uv.input_u64.clone().unwrap();
    ...
    let to = if input_u64 [1] == 0 { samples.len() } else {input_u64 [2] as usize };
    let input_f32 = uv.input_f32.clone().unwrap();
    if input_f32.len() < 3 {errMsg0("'input_f32' in Universum Vox sets 'scale' & 'amp', 'max_bottom'"); return}
    let scale = input_f32 [0];
    let amp: f32 = input_f32 [ 1 ];
    let max_bottom: f32 = input_f32 [ 2 ];
   // let vex: &Vec < f32 > = &uv.vex_f32.clone().unwrap() [0];
...
pub fn tune_wave_energy_mix_poly (mut samples: &mut [f32], uv: &crate::enums::universum_vox_morph) {
   // crate::cdsp::wave_energy_vox ( samples, &uv ); 
   let samples0: Vec < f32 > = if let Some (_vec) = &uv.input_bool {
        if _vec [0] {
            samples.into_iter().map (|x| *x ).collect() 
