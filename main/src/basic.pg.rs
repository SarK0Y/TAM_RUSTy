...
#[cfg(feature ="tst_macro")]
pub fn 
kbuild_page_(&mut self, ps: &mut
crate::_page_struct){
use rst_lex::lex::log_the_var;
use rst_lex::faav::log_attr;
let attrs = log_attr {path: "/dev/shm/build_page.log".to_string(), size: 3072 };let func_id = crate::func_id18::build_page_;
let __88value__359 = format! ("{:?}", func_id );;
log_the_var(2, "func_id", &__88value__359, &attrs);let mut try_entry = 0usize;
let __88value__359 = format! ("{:?}", try_entry );;
log_the_var(3, "try_entry", &__88value__359, &attrs);let mut num_files = crate::get_num_files(func_id);
let __88value__359 = format! ("{:?}", num_files );;
log_the_var(4, "num_files", &__88value__359, &attrs);let dbg_point =
    self.read_file("stop_point").trim_end().to_string();
let __88value__359 = format! ("{:?}", dbg_point );;
log_the_var(5, "dbg_point", &__88value__359, &attrs);#[cfg(feature="in_dbg")] if dbg_point == "001"{println!("stop 001");}while try_entry < 1_000
    {if crate::size_of_found_files() > 4u64 {break;} num_files =
        crate::get_num_files(func_id);
let __88value__359 = format! ("{:?}", num_files );;
log_the_var(12, "num_files", &__88value__359, &attrs);if num_files == 0i64{continue;}
        try_entry += 1;
let __88value__359 = format! ("{:?}", try_entry );;
log_the_var(15, "try_entry", &__88value__359, &attrs);}println!("{}", crate::get_full_path(func_id));let pg_info =
    (get_num_page(func_id), name_of_front_list("", false));
let __88value__359 = format! ("{:?}", pg_info );;
log_the_var(18, "pg_info", &__88value__359, &attrs);let GUARD_LAG =
    crate::smart_lags::screen_lag(None);
let __88value__359 = format! ("{:?}", GUARD_LAG );;
log_the_var(19, "GUARD_LAG", &__88value__359, &attrs);if let
    crate::enums::smart_lags::failed =
    crate::smart_lags::fork_lag_mcs_verbose(GUARD_LAG) {return;}if let
    crate::enums::smart_lags::too_small_lag(x) =
    crate::smart_lags::fork_lag_mcs_verbose(GUARD_LAG)
    {if x > GUARD_LAG / 10 {clear_screen() }}if
    !upd_screen_or_not(pg_info) && screen_state(None)
    {if name_of_front_list("", false) != "ls"
        {crate::lst::prnt_screen();return;}}let save_screen: String = take_list_adr("screen");
let __88value__359 = format! ("{:?}", save_screen );;
log_the_var(33, "save_screen", &__88value__359, &attrs);mk_empty_file(&save_screen);let mut save_screen =
    get_file(&save_screen).unwrap();
let __88value__359 = format! ("{:?}", save_screen );;
log_the_var(35, "save_screen", &__88value__359, &attrs);let nolog_redirect_out =
    Redirect::stdout(save_screen);let mut count_down = num_files;
let __88value__359 = format! ("{:?}", count_down );;
log_the_var(37, "count_down", &__88value__359, &attrs);if
    crate::size_of_found_files() == 0u64
    {println!("No files found");if
        !crate::checkArg("-dont-exit"){crate::C!(libc::exit(-1));}}let mut
    num_page;num_page = crate::calc_num_files_up2_cur_pg();
let __88value__359 = format! ("{:?}", num_page );;
log_the_var(44, "num_page", &__88value__359, &attrs);let mut num_cols;if ps.num_cols !=
    i64::MAX{num_cols =
        ps.num_cols;
let __88value__359 = format! ("{:?}", num_cols );;
log_the_var(47, "num_cols", &__88value__359, &attrs);}else{num_cols = crate::get_num_cols(func_id);
let __88value__359 = format! ("{:?}", num_cols );;
log_the_var(49, "num_cols", &__88value__359, &attrs);} let mut
    num_rows;if ps.num_rows !=
    i64::MAX{num_rows =
        ps.num_rows;
let __88value__359 = format! ("{:?}", num_rows );;
log_the_var(52, "num_rows", &__88value__359, &attrs);}else{num_rows = crate::get_num_rows(func_id);
let __88value__359 = format! ("{:?}", num_rows );;
log_the_var(54, "num_rows", &__88value__359, &attrs);} if
    ps.col_width != i64::MAX{crate::set_col_width(ps.col_width, func_id);} let
    num_items_on_pages = num_cols * num_rows;
let __88value__359 = format! ("{:?}", num_items_on_pages );;
log_the_var(57, "num_items_on_pages", &__88value__359, &attrs);let stopCode: String =
    crate::getStop_code__!();
let __88value__359 = format! ("{:?}", stopCode );;
log_the_var(58, "stopCode", &__88value__359, &attrs);let mut filename_str: String;let mut
    time_to_stop = false;
let __88value__359 = format! ("{:?}", time_to_stop );;
log_the_var(60, "time_to_stop", &__88value__359, &attrs);let mut nolog_row: Vec<CellStruct> = Vec::new();let
    mut row_cpy: Vec<String> = Vec::new();
let __88value__359 = format! ("{:?}", row_cpy );;
log_the_var(62, "row_cpy", &__88value__359, &attrs);let mut display_indx = 0i64;
let __88value__359 = format! ("{:?}", display_indx );;
log_the_var(63, "display_indx", &__88value__359, &attrs);for j
    in
    0..num_rows{for i in
        0..num_cols{let mut indx = i + num_cols * j + num_page;
let __88value__359 = format! ("{:?}", indx );;
log_the_var(66, "indx", &__88value__359, &attrs);let mut res: String
            ="".to_string();
let __88value__359 = format! ("{:?}", res );;
log_the_var(67, "res", &__88value__359, &attrs);let mut count_out = 77usize;
let __88value__359 = format! ("{:?}", count_out );;
log_the_var(68, "count_out", &__88value__359, &attrs);while res == "" &&
            count_out > 0
            {res = self.rec_from_front_list(indx, true);
let __88value__359 = format! ("{:?}", res );;
log_the_var(70, "res", &__88value__359, &attrs);count_out.dec();} if
            res == "no str gotten"
            {res = get_item_from_front_list(indx, true) }num_files =
            crate::get_num_files(func_id);
let __88value__359 = format! ("{:?}", num_files );;
log_the_var(74, "num_files", &__88value__359, &attrs);if num_files == indx ||
            "front list is empty" == res || "no str gotten" ==
            res.to_lowercase(){time_to_stop = true;
let __88value__359 = format! ("{:?}", time_to_stop );;
log_the_var(76, "time_to_stop", &__88value__359, &attrs);}let full_path = res;
let __88value__359 = format! ("{:?}", full_path );;
log_the_var(78, "full_path", &__88value__359, &attrs);display_indx = indx;
let __88value__359 = format! ("{:?}", display_indx );;
log_the_var(79, "display_indx", &__88value__359, &attrs);if
            !crate::C!(crate::swtch::local_indx(false)){display_indx = indx -
                num_page;
let __88value__359 = format! ("{:?}", display_indx );;
log_the_var(81, "display_indx", &__88value__359, &attrs);} let err_ret = std::ffi::OsString::from("");
let __88value__359 = format! ("{:?}", err_ret );;
log_the_var(82, "err_ret", &__88value__359, &attrs);let mut
            nolog_err_path = || -> &std::ffi::OsString{return &err_ret}; let
            mut filename = crate::Path::new(&full_path);
let __88value__359 = format! ("{:?}", filename );;
log_the_var(85, "filename", &__88value__359, &attrs);let
            nolog_filename_str0 = || ->
            String{let front_list =
                take_list_adr_env(&name_of_front_list("",
                false)).unreel_link_to_file();
let __88value__359 = format! ("{:?}", front_list );;
log_the_var(87, "front_list", &__88value__359, &attrs);if
                !crate::globs18::check_substrn01(&front_list,
                "history"){return
                    String::from(match
                    filename.file_name(){
                        Some(f) => f, None => nolog_err_path(),
                    }.to_str().unwrap()).as_str().strn();}else {return filename.as_os_str().to_str().unwrap().strn()};
            }; if filename.as_os_str().to_str() ==
            None{filename = crate::Path::new("")}if
            crate::globs18::eq_str(stopCode.as_str(),
            filename.as_os_str().to_str().unwrap()) == 0 && stopCode.len() ==
            filename.as_os_str().to_str().unwrap().len()
            {println!("{}", "caught".bold().green());time_to_stop = true;
let __88value__359 = format! ("{:?}", time_to_stop );;
log_the_var(98, "time_to_stop", &__88value__359, &attrs);break;} if
            crate::dirty!(){println!("cmp_str res {}",
                crate::globs18::eq_str(stopCode.as_str(),
                filename.as_os_str().to_str().unwrap()));println!("stop code {}, len {}; str {}, len {}", stopCode,
                stopCode.as_str().len(),
                filename.as_os_str().to_str().unwrap(),
                filename.as_os_str().to_str().unwrap().len());println!("{:?}", filename.file_name());}let mut fixed_filename: String =
            nolog_filename_str0().to_string();
let __88value__359 = format! ("{:?}", fixed_filename );;
log_the_var(105, "fixed_filename", &__88value__359, &attrs);crate::ins_newlines(crate::get_col_width(func_id).to_usize().unwrap(),
            &mut fixed_filename);if
            filename.is_dir(){filename_str
                =format!("{}: {}/", display_indx, fixed_filename);
let __88value__359 = format! ("{:?}", filename_str );;
log_the_var(108, "filename_str", &__88value__359, &attrs);}
            else{filename_str =
                format!("{}: {}", display_indx, fixed_filename);
let __88value__359 = format! ("{:?}", filename_str );;
log_the_var(110, "filename_str", &__88value__359, &attrs);} if
            filename_str == stopCode || filename_str ==
            "no str gotten"{return;} row_cpy.push(filename_str);if count_down
            <= 0 {time_to_stop = true;
let __88value__359 = format! ("{:?}", time_to_stop );;
log_the_var(115, "time_to_stop", &__88value__359, &attrs);break;} count_down -= 1;
let __88value__359 = format! ("{:?}", count_down );;
log_the_var(117, "count_down", &__88value__359, &attrs);}let count_pages = crate::get_num_files(func_id) /
        num_items_on_pages;
let __88value__359 = format! ("{:?}", count_pages );;
log_the_var(119, "count_pages", &__88value__359, &attrs);let mut new_row: Vec<Vec<CellStruct>> =
        Vec::new();
let __88value__359 = "".strn();//format! ("{:?}", new_row );;
log_the_var(120, "new_row", &__88value__359, &attrs);new_row.push(crate::pg18::cpy_row(&mut row_cpy));print_stdout(new_row.table().bold(true).foreground_color(Some(cli_table::Color::Blue)));if time_to_stop {break;}
    }drop(nolog_redirect_out);if crate::cmd_keys::screen_state(None) &&
    crate::smart_lags::fork_lag_mcs_bool(GUARD_LAG)
    {let screen = read_file("screen");
let __88value__359 = format! ("{:?}", screen );;
log_the_var(128, "screen", &__88value__359, &attrs);println!("{}\n{}", screen, crate::get_ask_user(func_id));}else {println!("{}", get_ask_user(func_id))}}}
