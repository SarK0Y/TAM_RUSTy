...
impl PQ {
    ...
    pub fn _(&self) -> Self { return self.clone() }
}
#[derive(Clone, PartialEq, Debug)]
...
pub unsafe fn npf_ext (n: rugint, rdx: u32) -> (rugint, rugint, String) {
   ...
   // errMsg0 (n_.as_str());
    while X.num1 () * Y.num1 () < n {
        ...
        if mark_positive_results.len() > 1 {ret = npf_recursion_lock (n.clone (), pq._() ); goto! ("End_npf_ext"); };
        pq = finally.0 [mark_j]._();
...
pub unsafe fn npf_forth (n: rugint, pq: &mut PQ) -> (rugint, rugint, String) {
    let fn_name = "npf ext".strn();
    let mut ret = (rugint::from (0), rugint::from (0), fn_name);
    let mut mark_j = 711_usize;
    let mut finally = vec_PQ { 0: Vec::<PQ>::new() };
