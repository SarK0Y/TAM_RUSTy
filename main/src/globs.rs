...
use crate::{
    ...
    core18::calc_num_files_up2_cur_pg,
    cpy_str, drop_ls_mode, errMsg0,
    exts::globs_uses,
    file_prnt, fix_num_files, from_ls_2_front, func_id18, get_arg_in_cmd, get_mainpath,
    get_path_from_prnt, get_path_from_strn, get_tmp_dir, i64_2_usize, ln_of_found_files,
    mark_front_lst, popup_msg,
    ps18::{get_prnt, set_ask_user, shift_cursor_of_prnt},
    read_file, read_file_abs_adr, read_front_list, read_prnt, repeat_char, rm_file, run_cmd0,
    run_cmd_out, run_cmd_str, save_file, save_file_abs_adr0, save_file_append_abs_adr,
    set_front_list, set_full_path, set_num_files, set_prnt, shm_tam_dir, split_once,
    swtch::{
        check_mode, front_list_indx, local_indx, set_user_written_path_from_prnt,
        set_user_written_path_from_strn, swtch_fn, user_wrote_path, SWTCH_RUN_VIEWER,
        SWTCH_USER_WRITING_PATH,
    },
    tailOFF, u64_from_strn,
...
pub fn rm_char_from_string(indx: usize, origString: &String) -> String {
    let len = origString.chars().count();
...
pub(crate) fn sieve_list(data: String) {
    if check_symb_in_strn(&data, "|") {
        return sieve_list0(data);
    }
    let data0 = data.replace("sieve ", "").replace(":+ ", "");
    let (mut opts, mut data0) = split_once(&data0, " ");
    if opts == "none".strn() || data0 == "none".strn() {
        set_ask_user(
            "example: {sieve -Ei some\\shere} or {:+ -Ei some\\shere}",
            5977871,
        );
    }
    let mut history_mode = "".strn();
    if check_substrn(&name_of_front_list("", false), "history") {
        history_mode = "_history".strn();
    }
    if opts == "none" {
        return;
    }
    ...
    #[cfg(feature = "mae")]
    {
        let filter_name = format!("filter{history_mode}");
        crate::cache::set_uid_cache(&filter_name);
    }
    ...
    let cmd = format!(
        "grep {} {} {} > {}",
        opts,
        crate::full_escape(&data0),
        found_files_path.clone().unreel_link_to_file0(),
        filter_file_path_tmp
    );
    run_cmd_str(cmd.as_str());
    ...
    let cmd = format!(
        "#filter as front\nln -sf {} {}",
        filter_file_path, found_files_path
    );
    run_cmd_str(cmd.as_str());
...
pub(crate) fn sieve_list0(data: String) {
    ...
    let cmd = format!(
        ...
        opts,
        data,
        found_files_path.clone().unreel_link_to_file(),
        filter_file_path_tmp
    );
    run_cmd_str(cmd.as_str());
    if match std::fs::metadata(&filter_file_path_tmp) {
...
pub fn bksp() -> String {
    let mut len = get_prnt(-3).chars().count();
    if len == 0 {
    ...
    if len == 0 {
        save_file("".to_string(), "prnt".to_string());
    }
    ////println!("ret {}", ret);
...
pub fn ins_last_char_to_string1_from_string1(indx: usize, origString: String) -> String {
    ...
    if len == 0 || len == 1 || indx == len - 1 {
        return origString.to_string();
    }
    let mut ret = String::new();
    len.dec();
    ...
    if crate::dirty!() {
        let msg = format!(
            "'ins_last_char_to_string1_from_string1 indx {} orig{} char0 {} orig len {}'",
            indx, origString, char0, len
        );
        //run_cmd0(&msg);
        println!("{}", &msg);
    }
...
pub fn ins_patch_to_string(indx: usize, origString: String, patch: &String) -> String {
    ...
    if crate::dirty!() {
        let msg = format!(
            "'ins_patch_to_string indx {} orig{} char0 {} orig len {}'",
            indx, origString, patch, len
        );
        //run_cmd0(&msg);
        println!("{}", &msg);
    }
    ret = format!(
        "{}{}{}",
        origString.substring(0, indx),
        patch,
        origString.substring(indx, len)
    );
    ret
}
pub fn ins_last_char_to_string1_from_string1_ptr(indx: usize, origString: &mut String) {
    ...
    origString.clear();
    origString.push_str(ret.as_str());
    ////println!("ret {}", ret);
}
...
pub fn eq_str(str1: &str, str2: &str) -> i64 {
    ...
    while i < str1_len && i < str2_len {
        if str1.chars().nth(i) == None || str1.chars().nth(i) == None {
            break;
        }
        if crate::dirty!() {
            println!(
                "eq_str char1 {:?} to char2 {:?} i {}",
                str1.chars().nth(i),
                str2.chars().nth(i),
                i
            );
...
pub fn eq_ansi_str(str1: &str, str2: &str) -> i64 {
    ...
    while i < str1_len && i < str2_len {
        ...
        if char_of_str1 == None || char_of_str2 == None {
            println!("char is None");
            break;
        }
        if crate::dirty!() {
            println!(
                "eq_ansi_str char1 {:?} to char2 {:?} i {}",
                str1.chars().nth(i),
                str2.chars().nth(i),
                i
            );
...
pub fn add_2_front_list(val: &str, func_id: i64) -> String {
    ...
    if !list_id.1 {
        set_ask_user("Can't access to Front list", func_id);
        return "!!no¡".to_string();
    }
...
pub fn raw_len_of_front_list() -> String {
    ...
    if !list_id.1 {
        set_ask_user("Can't access to Front list", -1);
        return "!!no¡".to_string();
    }
    let front_list = read_front_list();
    if front_list == "main0" {
        return len_of_main0_list();
    }
...
pub fn len_of_front_list() -> String {
    ...
    if num == "" {
        return len_of_list_wc(&name_front_list);
    }
    return num;
}
...
pub(crate) fn __get_proper_indx(indx: i64, fixed_indx: bool) -> (usize, i64) {
    ...
    if !unsafe { local_indx(false) } && fixed_indx == true {
        fix_inputed_indx += calc_num_files_up2_cur_pg();
        indx = fix_inputed_indx;
    }
...
pub fn set_main0_as_front() {
    crate::drop_ls_mode();
    mark_front_lst("main0");
    unsafe {
        lists("", MAIN0_, 0, SET_FRONT_LIST);
    }
}
pub fn set_ls_as_front() -> String {
    ...
    mark_front_lst("ls");
    let ret = || -> String {
        crate::C!(lists("", LS_, 0, SET_FRONT_LIST));
        return "ok".to_string();
    };
    //background_fixing();
    ret()
...
pub unsafe fn lists(val: &str, list: i64, indx: usize, op_code: i64) -> String {
    ...
    if list == LS_ {
        ...
        if op_code == LEN {
            return ln_of_found_files(usize::MAX).1.to_string();
        }
        if op_code == SET_FRONT_LIST {
    ...
    if list == FRONT_ {
        if op_code == GET {
            ...
            if FRONT.len() > indx && list == MAIN0_ {
                ...
                ret = cached_ln_of_found_files(indx).0;
            }
            return ret.to_string();
        } //return FRONT.get().unwrap()[indx].to_string()}
        if op_code == LEN {
            return ln_of_found_files(usize::MAX).1.to_string();
        } //return FRONT.get().unwrap().len().to_string()}
    }
    "wrong".to_string()
}
...
pub fn workaround_for_newlines_in_file_name (data: String) -> String {
    ...
    let ret = data.replace (r"\n", &change_newline);
    let ret = ret.replace (r"\0", "\n");
    return ret;
}
...
pub(crate) fn decode_sub_cmd(cmd: &String, sub_cmd: &str) -> (String, bool) {
    ...
    let mut full_sub_cmd = String::new();
    ...
    if crate::globs18::check_substrn(&cmd, sub_cmd) {
        ...
        if sub_cmd_val == "none" {
            errMsg0("Example of sub-command: >>>lst::name_of_list::<<<");
            return (cmd.to_string(), false);
        }
        full_sub_cmd = format!("{sub_cmd0}{sub_cmd_val}::");
        val = sub_cmd_val;
    }
    match sub_cmd {
        "lst::" => {
        ...
        "lun::" => {
            let state = sub_cmd_lst_uniq_num(&mut cmd, &full_sub_cmd, &val);
            return (cmd.strn(), state);
        }
        "lan::" => {
            let state = sub_cmd_last_lst(&mut cmd, &full_sub_cmd, &val);
            return (cmd.strn(), state);
        }

        "lne::" => {
            return sub_cmd_lst_not_exists(&cmd, &full_sub_cmd, &val);
        }
        "lab::" => {
            return sub_cmd_lst_always_blank(&cmd, &full_sub_cmd, &val);
        }
        _ => return (cmd.strn(), false),
    }
}
pub(crate) fn decode_sub_cmds(cmd: &String) -> String {
    if !cmd_decode_mode(None) {
        return cmd.strn();
    }
    ...
    loop {
        let ret = decode_sub_cmd(&ret0, "lan::");
        if ret.1 {
            ret0 = ret.0;
            continue;
        }
    ...
    #[cfg(feature = "in_dbg")]
    {
        save_file(ret0.clone(), "decoded_prnt".to_string());
        crate::report(&ret0, "decoded_prnt");
    }
    #[cfg(feature = "mae")]
    crate::cache::upd_uids_for_lsts();
    ret0
}
pub fn sub_cmd_last_lst<T: STRN + ToString + AsRef<str> + std::fmt::Display>(
    cmd: &mut String,
    full_sub_cmd: &String,
    val: T,
) -> bool {
    ...
    let mut lst = take_list_adr_env(&val.strn());
    let mut cnt = 0u64;
    while crate::Path::new(&lst).exists() {
        lst = take_list_adr_env(&format!("{val}{cnt}"));
        cnt.inc();
    }
    lst = take_list_adr_env(&format!("{val}{}", cnt.dec().dec()));
    *cmd = cmd.replace(full_sub_cmd, &lst);
    true
}
pub fn sub_cmd_lst_not_exists(cmd: &String, full_sub_cmd: &String, val: &String) -> (String, bool) {
    if *full_sub_cmd == "" {
        return (cmd.strn(), false);
    }
    let lst = take_list_adr_env(val);
    if !crate::Path::new(&lst).exists() {
        return (cmd.replace(full_sub_cmd, &lst), true);
    }
    (
        cmd.replace(full_sub_cmd, "--help /@@$%!/.~/hmm..here???"),
        false,
    )
}
pub fn sub_cmd_lst_always_blank(
    cmd: &String,
    full_sub_cmd: &String,
    val: &String,
) -> (String, bool) {
...
pub fn cmd_decode_mode(set: Option<bool>) -> bool {
    ...
    unsafe {
        if let Some(x) = set {
            state = x;
            errMsg0(&format!(
                "cmd decode mode is {state}.. Please, press any key to continue."
            ));
        }
        state
    }
...
pub(crate) fn take_list_adr_env(name: &str) -> String {
    match name {
        ...
        "main" => return take_list_adr("main"),
...
pub(crate) fn check_char_in_strn(strn: &String, is_there_ch: char) -> String {
    for ch in strn.chars() {
        if ch == is_there_ch {
            return String::from(is_there_ch);
        }
    }
    "no".to_string()
}
...
pub(crate) fn gen_win_title() -> String {
    let win_num = instance_num();
    let win_class_id = id_suffix();
    return format!("{win_num}{win_class_id}");
}
pub(crate) fn check_patch_mark(strn: &String) -> bool {
    let strn_len = strn.chars().count();
    let mut strn = strn;
    let patch_mark_len = "::patch".to_string().chars().count();
    if strn_len > patch_mark_len && strn.substring(strn_len - patch_mark_len, strn_len) == "::patch"
    {
        return true;
    }
    false
...
pub(crate) fn split_strn_by_not_escaped_spaces(strn: &String) -> Vec<String> {
    ...
    for v in 0..len {
        ...
        if ch == Some(' ') && v > 0 && strn.chars().nth(v - 1) != Some('\\') {
...
pub(crate) fn enum_not_escaped_spaces_in_strn_down_to(strn: &String, bar: usize) -> Vec<usize> {
    ...
    for v in 0..len {
        let ch = strn.chars().nth(v);
        if ch == Some(' ') && v > 0 && strn.chars().nth(v - 1) != Some('\\') && v > bar {
            vec.push(v.clone());
        }
    }
    vec
...
pub(crate) fn Users_home_dir() -> String {
    ...
    if home.substring(0, 1) != "/" {
        errMsg0("Can't get User's home direcrory");
        return "".strn();
    }
    home
}
...
pub(crate) fn load_fish_history() {
    ...
    let bkp_fish = format!(
        "cat {orig_fish}|grep -Ei '\\s*-\\s*cmd:'|sed 's/\\s*-\\s*cmd:/term /g'|uniq > {cpy_to}"
    );
